...
def blockBot(hexMap, depth, moves, freeBlock, timeStart=None, score=6, quie=False): # Instead of quie maybe depth? Start at 1, then when this called again it'll be 0##############################
    ...
    for hexC in (hexRange or possList):
        if timeStart and time.time() - timeStart >= .1:
            return maxScore
        new_score, uselessBlock, shortestStep = score, False, 2
        futScoreAvg = 0
        # Copy the game state
        hexMap.blockTile(hexC, draw=False)
        if not freeBlock: new_score, uselessBlock, shortestStep = scoreTile(hexMap, coord, score, winCountB, tilesB, max(freeBlock-1, 0)) # Get the new score for blockBot
        ...
        #if depth == 1 and not quie: print(time.time() - timeNow)
...
if __name__ == "__main__":
    ...
    losses = []
    prev = [29, 31]
    game = False # Not playing the game and wanting it visualized
    for index, gameState in enumerate(gamesList): # 8:9, 31:
        #if prev and index not in prev: continue
