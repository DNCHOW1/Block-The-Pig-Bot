...
class HexMap():
    ...
    def floodFill(self, start, bound=None): # Good for the beginning 2 step->3 step; opt is max amount out you want to go
        ...
        countRange = 3
        countTotal = 0
        opt = max(3, distLook.get(bound, bound)) if bound else 10
        ret = None # If choke point present or not
        i = 1
        while i <= opt:
            fringes.append([])
            ...
            if len(fringes[-1]) == 1 and not ret and not winCount: ret = fringes[-1]
            i += 1
        if len(winCount) == 1: winCount[100] = 0
        return (ret or chain(*fringes[1:4]), winCount, countTotal)
...
if __name__ == "__main__":
    ...
    for index, gameState in enumerate(gamesList): # 21-26
